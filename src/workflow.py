...
def decide_action(state: State) -> str:
    ...
    Verifica o estado atual da conversa para determinar qual o próximo nó
    (agente) a ser executado. Se uma resposta já existir no estado 
    (vindo de um caso geral tratado pelo supervisor), encerra o fluxo.
    Caso contrário, retorna o nome da rota definida pelo supervisor.

    Args:
        state (State): O estado atualizado da conversa.
...
def build_workflow() -> CompiledStateGraph:
    ...
    Define os nós do sistema (o supervisor e os diversos agentes especialistas),
    estabelece o ponto de entrada (START) e configura o roteamento condicional
    baseado na função `decide_action`. 
