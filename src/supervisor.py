...
def supervisor(state: State) -> State:
    ...
    tratar a pergunta do usuário.

    Este nó do LangGraph analisa a pergunta (query) do usuário e o histórico do chat.
    Ele utiliza o LLM para classificar a intenção e retorna uma palavra-chave
    (como 'detalhe_tecnico', 'tickets', etc.) que determina a próxima etapa no fluxo,
    ou gera uma resposta direta para saudações ou perguntas fora do escopo.

    Args:
        state (State): O estado atual da conversa, contendo a query do usuário.

    Returns:
        State: O estado atualizado. Se a pergunta requerer um especialista, 
